@if ($paginator->lastPage() > 1)
    ...
    @elseif($paginator->currentPage() == $paginator->lastPage())
        <a class="nav-link-next nav-item nav-link rounded" href="{{ $paginator->url($paginator->currentPage() - 1)}}">Previous<i class="arrow-prev fas fa-long-arrow-alt-left"></i></a>
        <a class="nav-link-prev nav-item nav-link d-none rounded-left" href="#">Next<i class="arrow-next fas fa-long-arrow-alt-right"></i></a>
    @else
